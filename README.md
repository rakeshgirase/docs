...
Open a console box and clone the required git repositories:
...
Start the docker image created in the previous step by specifying the **full path** to the `ns-docs` folder:

```bash
sudo docker run -v /home/ubuntu/ns-docs:/root -p 9192:9192 -t ns-docs:1.0
```

Due to the [poor performance of mounted volumes on Mac OS](https://docs.docker.com/docker-for-mac/osxfs/#performance-issues-solutions-and-roadmap) you may use the [`delegated`](https://docs.docker.com/docker-for-mac/osxfs-caching/#delegated) or [`cached`](https://docs.docker.com/docker-for-mac/osxfs-caching/#cached) strategies:

```bash
sudo docker run -v /home/ubuntu/ns-docs:/root:delegated -p 9192:9192 -t ns-docs:1.0
```
